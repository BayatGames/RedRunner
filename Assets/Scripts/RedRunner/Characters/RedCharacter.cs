...
namespace RedRunner.Characters
{
    ...
    public class RedCharacter : Character
    {
        ...
        // THE JUMP STRENGTH FOR JUMPING RIGHT AFTER A ROLL
        [SerializeField]
        protected float m_RollJumpStrength = 14f;

        // THE TIME WINDOW IN SECONDS FOR EXECUTING JUMP + ROLL ABILITY (Basically how long the roll is)
        [SerializeField]
        protected float m_RollJumpTimeWindow = 1f;
        [SerializeField]
        ...
        // The variable to detect if we are rolling or not
        protected bool m_isRolling;
        // The variable that counts how long we are rolling and if we can use the jump + roll feature
        ...
        void Update()
        {
            ...
            {
                // Sets the rolling variable to true
                m_isRolling = true;
                rollTimer = 0;
            ...
            // ROLL TIMER CODE
            // First, check if the player is rolling (this saves a bit on performance because we don't need to run this when the player isn't rolling)
            if (m_isRolling)
            {
                // Check if the counter is below the treshold
                if (rollTimer < m_RollJumpTimeWindow)
                {
                    // Add time to the timer (time since last frame)
                    rollTimer += Time.deltaTime;
                }
                // If the rolltimer has expired, set rolling to false
                else
                {
                    m_isRolling = false;
                }
        ...
        public override void Jump()
        {
            if (!IsDead.Value)
            {
                if (m_GroundCheck.IsGrounded)
                {
                    ...
                    // If the character is rolling, set the jump height to the roll + jump ability jump height
                    if (m_isRolling)
                    {
                        velocity.y = m_RollJumpStrength;
                    }
